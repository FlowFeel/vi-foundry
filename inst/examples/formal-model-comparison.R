...
cat(sprintf("  dep       = %+.3f  (p = %.4f)  <- VI predicts > 0  [WRONG SIGN]\n",
    coef(fit_a0)[2], s_a0$coefficients[2,4]))
cat(sprintf("  para      = %+.3f  (p = %.4f)  <- VI predicts < 0  [ns]\n",
...
cat(sprintf("  dep       = %+.3f  (p = %.4f)  <- VI predicts > 0  [CORRECT, sig]\n",
    coef(fit_a)[2], s_a$coefficients[2,4]))
cat(sprintf("  para      = %+.3f  (p = %.4f)  <- VI predicts < 0  [CORRECT, sig]\n",
...
cat("  para p < 0.0001, cross-kingdom ρ = +0.755 — all matching VI predictions.\n")
