...
This document presents the **three-move analysis** for deriving the functional form of the Valence-Ingression (VI) effect. The monograph proposes that VI's effect on genome retention follows a substrate-shift equation: α(x) = −k_ecol + k_cult · σ((x − x*)/s). The question is whether the data falls on a recognizable curve — and if so, whether that curve matches the theory's prediction.
...
2. **Fisher (decompositional):** Partition Sodalis retention variance into VI + essentiality + drift + hitchhiking.
...
Plot VI effect size (ρ) against niche dependency parameter (θ) across all systems. θ measures the degree to which the environment (host) provides metabolic requirements — the control parameter. ρ measures how strongly gene-level dependency predicts retention — the order parameter.
...
**Key finding:** Sodalis (θ=0.04) and Buchnera (θ=0.50) have nearly identical ρ despite a 12× difference in θ. The VI effect saturates immediately once symbiosis begins.
...
Within the Sodalis system (1,366 genes matched to iJO1366), partition retention variance into components: VI (dependency score), selection (essentiality), drift (baseline), and hitchhiking (residual).

### Results

| Component | Effect size | Metric |
|-----------|------------|--------|
| VI (high-dep minus low-dep retention) | +0.398 | Difference in retention rates |
...
**Key finding:** Adding essentiality to VI gives **zero improvement** in AUC (0.656 → 0.656). VI subsumes essentiality entirely. The genes that are metabolically essential ARE the high-dependency genes — there is no independent selection signal beyond what dependency captures.
...
The jump from 34% to 78% between low and mid is the VI signal — it is not gradual but a **step within the system**. Genes with any non-trivial dependency (above 0.01) are retained at roughly twice the rate of zero-dependency genes.
...
The raw VI effect is constant across symbiotic systems at different dependency levels. This is consistent with a **first-order phase transition** (discontinuous jump), not a second-order transition (continuous sigmoid).

### Physics Attractor Mapping

The step-function finding maps directly onto physics attractor theory:

| VI concept | Physics analog |
|------------|---------------|
| θ (niche dependency) | T/Tc (reduced temperature) |
| ρ (VI effect) | M (magnetization, order parameter) |
...
Below θ*, the system is in the "free-living" attractor basin (full genome, no VI). Above θ*, it jumps to the "symbiotic" attractor basin (reducing genome, VI active). The jump is discontinuous — like water to ice, not like iron cooling through the Curie point.
