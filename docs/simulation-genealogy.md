...
lineage: phosphene dashboards → VI simulacra → cross-filter dashboard
...
phosphene D3 dashboards (2014) — simulation as design probe
        │
        ▼
VI simulacra (2025) — simulation as epistemic instrument
        │
        ▼
...
## Phase 2: Simulation as Epistemic Instrument (VI simulacra, 2025)

The VI framework introduced simulacra — simulations that test theoretical claims. The `inst/simulacra/` directory in vi-foundry contains generative scripts:
...
Simulation has always served a dual role: it **designs** (what should this be?) and it **proves** (is this claim true?). The phosphene dashboards were design simulations. The VI simulacra were epistemic simulations. The cross-filter dashboard is both — and that unification is the contribution.

The genealogy reveals that "proving ground" is the right metaphor. A proving ground is where you test something before you deploy it. The phosphene tradition proved the design. The VI tradition proves the theory. The dashboard proves both — and it does it in the browser, interactively, with the spec watching.
...
| VI simulacra `generate_relaxation.R` (2025) | Epistemic | Bi-exponential recovers its own params |
| VI genealogy Stage 6 (2025) | Epistemic | ΔAIC = −89.99, param recovery 9-14% |
...
*This document is part of the VI Foundry genealogy series. The prior is phosphene (2014). The methodology is simulation as proving ground. The inheritance: design probe + epistemic instrument = proving ground.*
