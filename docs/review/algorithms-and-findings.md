# The vi-foundry Algorithms, Applications, and Findings — A Formal Survey
...
This is the formal, literate survey of the `vi-foundry` computational pipeline:
...
Valence-Ingression (VI) framework.
...
"confirms VI" in its broadest form. The foundry confirms (or fails to confirm)
...
different pattern, so the result actually favours VI over the competitor." A
test can corroborate VI without discriminating it (T1, T2, T7) — that is
...
by what they can *establish*: first the tests that **discriminate** VI from a
...
a small permutation p refutes random loss and supports VI. ρ ≈ 0 supports the
competitor.
...
distinguishes VI from substrate-independence (a positive plant slope yielding
...
**The prediction.** VI predicts that **niche breadth** predicts gene loss
...
**discriminates** VI from drift: if niche subsumes Ne, drift is insufficient.
...
Niche R² exceeds Ne R², and AIC favours the niche model by 13.6. **VI is
...
VI prediction. The function regressed the *core* genome size (not the pan-
...
exact opposite of VI, purely from a wiring bug. The fix (pan-genome response,
...
power. This is a genuine point in VI's favour and against the pure-drift
...
**The prediction.** VI predicts that pan-genome openness (fluidity) tracks
**lifestyle** (commensal vs. free-living) rather than Ne alone. Like T4, this
discriminates VI from the Ne-only drift model.
...
Lifestyle R² exceeds Ne R². **VI is confirmed.** This entry **passes** the
regression gate.
...
foundry's most robust empirical point for VI over drift.

### T3 — Endosymbiont biphasic genome reduction

**The prediction.** VI predicts that genome reduction in obligate endosymbionts
...
decay. This discriminates VI from both competitors.
...
competitor), and logistic (biphasic/VI). R² is computed as 1 − RSS/TSS;
...
**The prediction.** VI predicts that plastome size correlates with parasitism
...
gradient through a different mechanism. This test **corroborates** VI but does
not **discriminate** it.
...
with VI — but because relaxed selection predicts the same gradient, it cannot
*favour* VI over the competitor. It is necessary-but-not-sufficient evidence:
VI requires this gradient, but so does the competitor. The discriminating
...
T1, this is consistent with VI but does not discriminate it. The per-family
...
**The prediction.** VI predicts that metabolic function loss in the LTEE
...
co-segregation. This corroborates VI but does not discriminate it (both
...
VI drift account (unused genes drift; their loss is not tied to adaptive
events). It is reported as *suggestive* due to a hitchhiking confound and an
arbitrary window, and it does not discriminate VI from independent assortment
...
**The prediction.** After a substrate shift, VI predicts that innovations
...
**The prediction.** VI predicts that capacity reallocation is **irreversible**:
...
**The prediction.** VI's integration-depth and commitment dynamics are
...
drafts under `drafts/vi-econ-papers/`.
...
to a regression-gate dataset. They demonstrate that the VI dynamical
...
3. **Empirical corroboration of three discriminating predictions.** Three tests
   that *distinguish* VI from a named competitor now run on real data and
   favour VI:
   - **T4 (niche vs. Ne):** niche R² (0.364) > Ne R² (0.257), AIC favours niche
     ...
     explanatory power. *(Was inverting VI before the bug fix.)*
...
4. **Honest reporting.** The regression gate does not pass silently. It reports
   ...
   *contradicting* VI from a wiring bug; that is now fixed, and zero entries
   contradict VI.
...
- **T1, T2, T7 corroborate but do not discriminate.** They are consistent with
  VI but equally consistent with relaxed selection, stochastic loss, or
  independent assortment. They are necessary-but-not-sufficient; they cannot
  favour VI over the competitors.
...
genuine point in VI's favour: pure drift is insufficient. The formal model
...
| T4 niche-vs-Ne | **yes** | skip (drift; **VI confirmed** — niche subsumes Ne) |
