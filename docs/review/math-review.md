# The vi-foundry Math Review — Implementation vs. Mathematics vs. Claims
...
A line-by-line audit of the mathematics in `vi-foundry`: for each analysis
...
is in fact *consistent* with the VI drift prediction (unused genes drift, so
