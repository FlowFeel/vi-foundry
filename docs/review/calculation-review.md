# The vi-foundry Calculation Review — A Literate Walkthrough
...
every analysis in `vi-foundry` against its baseline oracle, the diagnosis of
...
[`vi-foundry-phased-breakdown.md`](vi-foundry-phased-breakdown.md) (which
...
> *discriminate* VI from its competitors now pass the regression gate (T5,
> formal_model), and a third confirms the VI prediction (T4). Before the
> review, one entry (T4) was actively *contradicting* VI purely from a
...
1. **The prediction** — what VI claims, and what the competitor claims.
...
## T4 — Niche breadth vs. Ne (the one that inverted VI)

### The prediction

VI predicts that **niche breadth** predicts gene loss (pan-genome size) better
...
**does** distinguish VI from drift: if niche subsumes Ne, drift is
insufficient; if Ne wins, VI's niche claim is unsupported.
...
Niche R² (0.343) exceeds Ne R² (0.198). VI wins.
...
(Mb)` — the *core* genome size. But the VI prediction is about **gene loss**,
...
opposite of the VI prediction. This was not a subtle drift or a data-version
...
(242 < 256). **VI is confirmed.** The residual drift from the oracle (0.364 vs.
...
VI predicts that pan-genome openness (fluidity) tracks **lifestyle** (commensal
vs. free-living) rather than Ne alone. Like T4, this distinguishes VI from the
Ne-only drift model.
...
`TRUE`. **VI is confirmed.** This entry now **passes** the regression gate.
...
retained at 1.0). This is the theoretical signature that distinguishes VI from
...
VI predicts that genome reduction in obligate endosymbionts follows
...
— consistent with the VI drift prediction. *Action: update the oracle p-value
...
| **T4 niche-vs-Ne** | **skip (Ne WINS — inverts VI)** | **skip (niche wins; VI confirmed)** |
...
| Entries contradicting VI | 1 (T4) | **0** |
