...
| [`formal-model-reproduction.md`](formal-model-reproduction.md) | Deep-dive reproduction of the author's broken GLM. **Root cause:** a data-flattening bug (`as.vector(t(retention))` misaligns `dep` and `retention`). **Fix:** remove one `t()`. With correct alignment, the additive GLM gives `dep = +0.84` (p = 0.0008), `para` p < 0.0001, cross-kingdom ρ = +0.755 — all matching VI. Contrasts five model specs (misaligned GLM, corrected additive GLM, interaction GLM, threshold fit, foundry ODE) against the foundry's non-empirical simulation. **Now implemented:** the corrected GLM is restored as `empirical_formal_model()`, the 8×6 matrix is bundled in `data/`, and it is wired into the pipeline and regression gate. Includes executable companion (`inst/examples/formal-model-comparison.R`) and formal deprecation note. Records the finding as **Remark R7** in the review. |
