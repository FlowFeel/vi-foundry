---
uri: vi-foundry/design-spec
owner: edphos
status: draft
updated: 2026-08-18
---

# VI Foundry — Design Specification
...
|--------|---------------|-------|
| `inst/simulacra/` | Synthetic data generators (VI claims) | one per generator |
...
```
Ising (MC) → Landau (mean-field) → Cusp (bifurcation) → ... → VI (step)
```
...
3. The final stage (VI step) is tested directly (step recovery simulacrum)
...
| Drift-selection → VI | Stage 5 → Stage 6 | T2: does WF produce ρ_sat ≈ 0.35? |
